            ...
            <div class="header">
                <table class="info-table">
                    <tr>
                        ...
                        <th><span>{{ __f('Invoice No Title') }} : {{ $invoice->invoice_id }}</span></th>
                    </tr>
                </table>
            </div>
            <div class="section">
                <table class="info-table">
                    ...
                    <tr>
                        ...
                        <td>{{ __f('Name Title') }} : {{ $invoice->customer_name }} </td>
                    </tr>
                    <tr>
                        <td>{{ config('settings.company_email') ?? '' }}</td>
                        <td>{{ __f('Phone Title') }} : {{ $invoice->customer_phone }} </td>
                    </tr>
                    <tr>
                        <td>{{ config('settings.company_cell') ?? '' }}</td>
                        <td>{{ __f('Address Title') }} : {{ $invoice->customer_address }}</td>
                    </tr>
                    <tr>
                        <td></td>
                        <td>{{ __f('Created Date Title') }} : {{ formatDateByLocale(\Carbon\Carbon::parse($invoice->create_date)->format('d-m-Y')) }}</td>
                    </tr>
                    <tr>
                        <td></td>
                        <td>{{ __f('Due Date Title') }} : {{ formatDateByLocale(\Carbon\Carbon::parse($invoice->due_date)->format('d-m-Y')) }}</td>
                    </tr>
                </table>
            </div>
                ...
                <tr>
                    ...
                    <th>
                        <div class="total-calculation">
                            <p>{{ __f('Sub Total Title') }} : {{ convertToLocaleNumber($sub_total) }} {{ currency() }}</p>
                            <p>{{ __f('Discount Title') }} : {{ convertToLocaleNumber($invoice->discount ?? 0) }} {{ currency() }}</p>
                            <p>{{ __f('Grand Total Title') }} : {{ convertToLocaleNumber($grand_total) }} {{ currency() }}</p>
                            <p>{{ __f('Paid amount Title') }} : {{ convertToLocaleNumber($invoice->paid_amount) }} {{ currency() }}</p>
                            <p>{{ __f('Due amount Title') }} : {{ convertToLocaleNumber($due_amount) }} {{ currency() }}</p>
                        </div>
                    </th>
                </tr>
            </table>
