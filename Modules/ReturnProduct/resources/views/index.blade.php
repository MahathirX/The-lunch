            ...
            <div class="bg-white border-bottom-0 pb-4 d-flex justify-content-between align-items-center flex-row">
                ...
                <div class="email-template-search-bar btn-group d-flex align-items-center">
                    ...
                    <input type="text" id="datatable-search" class="h-100 border w-100 py-2 px-3"
                        placeholder="{{ __f("Invoice id Name Phone Placeholder") }}">
                </div>
            </div>
            <table id="return_product_list" class="display" style="width:100%">
                <thead>
                    <tr>
                        <th>{{ __f("SL Title") }}</th>
                        <th>{{ __f("Invoice Id Title") }}</th>
                        <th>{{ __f("Created Date Title") }}</th>
                        <th>{{ __f("Name Title") }}</th>
                        <th>{{ __f("Phone Title") }}</th>
                        <th>{{ __f("Refunded Quentity Title") }}</th>
                        <th>{{ __f("Refunded Amount Title") }}</th>
                        <th>{{ __f("Previous Paid Amount Title") }}</th>
                        <th>{{ __f("Previous Due Title") }}</th>
                        <th>{{ __f("Discount Title") }}</th>
                        <th>{{ __f("Grand Refunded Amount Title") }}</th>
                        <th>{{ __f("Grand Pay Amount Title") }}</th>
...
<script>
    ...
    var tables = $('#return_product_list').DataTable({
        processing: true,
        serverSide: true,
        order: [],
        bInfo: true,
        ...
        pageLength: 25,
        ajax: {
            url: "{{ route('admin.returnproduct.get.data') }}",
            type: "GET",
            dataType: "JSON",
            data: function(d) {
                d._token = _token,
        ...
        columns: [{
            ...
            {
                data: 'grand_refunded_amount'
            },
            {
                data: 'grand_pay_amount'
            },
            {
                data: 'note'
            },
