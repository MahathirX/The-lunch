        ...
        <div class="card">
            <div class="card-heading d-flex justify-content-between align-items-center  purchas-invoice-show">
                ...
                <div class="d-flex gap-2 mt-3 px-2 no-print">
                        <a target="_blank" href="{{ route('admin.returnpurchase.download.print', ['id' => $returnPurchaseProduct->id]) }}"
                ...
                <div class="invoice-box p-4">
                    <div class="row">
                        ...
                        <div id="text-center" class="col-12 col-md-6 text-end">
                            <span>{{ __f('Invoice Title') }}: {{ '#'.$returnPurchaseProduct->invoice_id }}</span><br>
                            {{ __f('Created Date Title') }}: <span class="discount">{{ formatDateByLocale(\Carbon\Carbon::parse($returnPurchaseProduct->create_date)->format('d-m-Y')) }}</span><br>
                            <span id="hr-border"></span>
                            <span>{{ $returnPurchaseProduct->supplier->name ?? '-----' }}</span><br>
                            <span>{{ $returnPurchaseProduct->supplier->phone ?? '-----' }}</span><br>
                            <span>{{ $returnPurchaseProduct->supplier->address ?? '-----' }}
                        </div>
                    </div>
                </div>
                <div class="invoice-box table-responsive">
                    <table cellpadding="0" cellspacing="0">
                        ...
                        @php
                            $sub_total = 0;
                        @endphp
                        @foreach ($returnPurchaseProduct->returnpurchasedetails as $item)
                            <tr id="productborder">
                        ...
                        <div class="card-summary">
                            <ul class="list-group list-group-flush small">
                                ...
                                <li class="list-group-item d-flex justify-content-between">
                                    ...
                                    <span>{{ convertToLocaleNumber($returnPurchaseProduct->total_amount + $returnPurchaseProduct->purchasinvoice->admin_sub_total) }} {{ currency() }}</span>
                                </li>
                                <li class="list-group-item d-flex justify-content-between">
                                    <strong>{{ __f('Total Due Amount Title') }} :</strong>
                                    <span class="grand-total"> {{ convertToLocaleNumber($returnPurchaseProduct->due_amount ?? 0) }} {{ currency() }}</span>
                                </li>
                                <li class="list-group-item d-flex justify-content-between">
                                    <strong>{{ __f('Discount Amount Title') }}:</strong>
                                    <span class="grand-total"> {{ convertToLocaleNumber($returnPurchaseProduct->discount ?? 0) }} {{ currency() }}</span>
                                </li>
                                <li class="list-group-item d-flex justify-content-between">
