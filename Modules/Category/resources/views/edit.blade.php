                ...
                <form id="categoryEditForm" action="{{ route('admin.category.updates', ['id' => $category->id]) }}"
                    ...
                    <div class="row">
                        <x-form.selectbox parantClass="col-12 col-md-6" class="form-control py-2" name="parent_id"
                            ...
                            @forelse ($categories as $categorie)
                                <option value="{{ $categorie->id ?? '' }}" {{ $category->parent_id == $categorie->id ? 'selected' : '' }}>{{ $categorie->name ?? '' }}</option>
                            @empty
                        ...
                        <x-form.selectbox parantClass="col-12 col-md-6" class="form-control py-2" name="submenu_id"
                            ...
                            @forelse ($subcategories as $subcategorie)
                                <option value="{{ $subcategorie->id ?? '' }}" {{ $category->submenu_id == $subcategorie->id ? 'selected' : '' }}>{{ $subcategorie->name ?? '' }}</option>
                            @empty
                                <option value="" disabled class="text-danger">{{  __f("No Sub Category Found Text") }}</option>
                            @endforelse
                        </x-form.selectbox>
                    </div>
                    <div class="row">
                        <x-form.selectbox parantClass="col-12 col-md-6" class="form-control py-2" name="home_page_show"
                            ...
                            <option value="0"{{ $category->home_page_show == '0' ? 'selected' : '' }}>{{ __f('No Text') }}</option>
                            <option value="1"{{ $category->home_page_show == '1' ? 'selected' : '' }}>{{ __f('Yes Text') }}</option>
                        </x-form.selectbox>
                    ...
                    <div class="row">
                        ...
                        <x-form.textbox labelName="{{ __f('Category Tag Title') }}" parantClass="col-12 col-md-6" name="tag"
                            ...
                            value="{{  $category->tag ?? old('tag') }}"></x-form.textbox>
                    </div>
...
@push('scripts')
    <script>
        ...
        if (categoryImage != '') {
            var myFData = "{{ asset($category->image ?? '') }}";
            $(function() {
                ImagePriviewUpdate('first__image', 'picture__first', '{{ __f("Category Image Placeholder") }}', myFData);
            });
        }
