...
@section('add_button')
    <div>
        @if(Auth::check() && Auth::user()->role_id == 3)
            <a href="{{ route('staff.supplier.create') }}"
            ...
            <div class="bg-white border-bottom-0 pb-4 d-flex justify-content-between align-items-center flex-row">
                ...
                <div class="email-template-search-bar btn-group d-flex align-items-center">
                    ...
                    <input type="text" id="datatable-search" class="h-100 border w-100 py-2 px-3"
                        placeholder="{{ __f('Name Phone Address Placeholder')}}">
                </div>
            </div>
            <table id="supplier_id" class="display" style="width:100%">
                <thead>
                    <tr>
                        <th>{{ __f("SL Title") }}</th>
                        <th>{{ __f("Name Title") }}</th>
                        <th>{{ __f("Phone Title") }}</th>
                        <th>{{ __f("Supplier Email Title") }}</th>
                        <th>{{ __f("Company Name Title") }}</th>
...
<script>
    ...
    var tables = $('#supplier_id').DataTable({
        processing: true,
        serverSide: true,
        order: [],
        bInfo: true,
        ...
        pageLength: 25,
        ajax: {
            url: "{{ route('admin.supplier.get.data') }}",
            type: "GET",
            dataType: "JSON",
            data: function(d) {
                d._token = _token,
