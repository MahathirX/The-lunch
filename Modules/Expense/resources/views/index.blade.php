...
@section('add_button')
    <div>
        @if(Auth::check() && Auth::user()->role_id == 3)
            <a href="{{ route('staff.expense.create') }}"
            ...
            <div class="bg-white border-bottom-0 pb-4 d-flex justify-content-between align-items-center flex-row">
                ...
                <div class="email-template-search-bar btn-group d-flex align-items-center">
                    ...
                    <input type="text" id="datatable-search" class="h-100 border w-100 py-2 px-3"
                        placeholder="{{ __f("Name Code Placeholder") }}">
                </div>
            </div>
            <table id="expenseId" class="display" style="width:100%">
                <thead>
                    <tr>
                        <th>{{ __f("SL Title") }}</th>
...
<script>
    ...
    var tables = $('#expenseId').DataTable({
        processing: true,
        serverSide: true,
        order: [],
        bInfo: true,
        ...
        pageLength: 25,
        ajax: {
            url: "{{ route('admin.expense.get.data') }}",
            type: "GET",
            dataType: "JSON",
            data: function(d) {
                d._token = _token,
