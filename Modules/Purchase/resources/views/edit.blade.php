...
<section>
    <div class="card">
        <div class="card-heading">
            <h3 class="p-2">{{ __f('Purchase Edit Title') }}</h3>
        </div>
        <div class="card-body">
            <form id="PurchaseEditForm" action="{{ route('admin.purchase.updates',['id' => $editinvoice->id]) }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <x-form.textbox labelName="{{ __f('Choose Date Label Title') }}" parantClass="col-12 col-md-3" name="invoice_date"
                         errorName="invoice_date" class="py-2"
                        value="{{ $editinvoice->invoice_date ??  old('invoice_date') }}" type="date"></x-form.textbox>

                    <x-form.selectbox parantClass="col-12 col-md-3 overflow-hidden" class="form-control selectpiker" name="supplier_id" id="supplier_id"
                    ...
                    <x-form.selectbox parantClass="col-12 col-md-3" class="form-control py-2" name="status"
                        required="required" labelName="{{ __f('Purchase Status Label Text') }}" errorName="status">
                        <option value="3" {{ $editinvoice->status == '3' ? 'selected' : '' }}>{{ __f('Purchase Status Received Title') }}</option>
                        <option value="2" {{ $editinvoice->status == '2' ? 'selected' : '' }}>{{ __f('Purchase Status Pending Title') }}</option>
                        <option value="1" {{ $editinvoice->status == '1' ? 'selected' : '' }}>{{ __f('Purchase Status Ordered Title') }}</option>
                        <option value="0" {{ $editinvoice->status == '0' ? 'selected' : '' }}>{{ __f('Purchase Status Partial Title') }}</option>
                    </x-form.selectbox>
                    <x-form.selectbox parantClass="col-12 col-md-3" class="form-control py-2" name="purchase_type"
                        required="required" labelName="{{ __f('Purchase Type Title') }}" errorName="purchase_type">
                        <option value="1" {{ $editinvoice->purchase_type == '1' ? 'selected' : '' }}>{{ __f('Purchase Supplier Title') }}</option>
                        <option value="0" {{ $editinvoice->purchase_type == '0' ? 'selected' : '' }}>{{ __f('Purchase Local Title') }}</option>
                    </x-form.selectbox>
                </div>
                        ...
                        <table cellpadding="0" cellspacing="0">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ __f('Buy Price Title') }}</th>
                                    <th>{{ __f('Original Sub Total Title') }}</th>
                            ...
                            <tbody>
                                ...
                                <tr class="item">
                                    ...
                                    <td>
                                        <input name="buy_price[]" type="number" class="buy_price" required value="{{ $invoice->buy_price ?? 0 }}"/>
                                    </td>
                                    <td class="admin_total w-7"> {{ $invoice->admin_sub_total ?? 0 }} {{ currency() }}</td>
                                    <td class="total w-7"> {{ $invoice->sub_total ?? 0 }} {{ currency() }}</td>
                                    <td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="text-danger">{{ __f("No product Found Title") }}</td>
                                    </tr>
                                @endforelse
                            </tbody>
                            <tr>
                                ...
                                <div class="card-summary">
                                    <ul class="list-group list-group-flush small">
                                        ...
                                        <li class="list-group-item d-flex justify-content-between">
                                            ...
                                            @if ($editinvoice->paid_amount != null && $editinvoice->discount != null)
                                              <span class="previou-sub-total"> {{ ($editinvoice->sub_total - $editinvoice->discount) -  $editinvoice->paid_amount }} {{ currency() }}</span>
                                            @elseif ($editinvoice->paid_amount != null)
                                              <span class="previou-sub-total"> {{ $editinvoice->sub_total - $editinvoice->paid_amount }} {{ currency() }}</span>
                                            @elseif($editinvoice->discount != null)
                                              <span class="previou-sub-total"> {{ $editinvoice->sub_total - $editinvoice->discount }} {{ currency() }}</span>
                                            @else
                                              <span class="previou-sub-total"> {{ $editinvoice->sub_total }} {{ currency() }}</span>
                                            @endif
                                        </li>
                                        <li class="list-group-item d-flex justify-content-between">
                                        ...
                                        <li class="list-group-item d-flex justify-content-between">
                                            ...
                                            @if ($editinvoice->paid_amount != null && $editinvoice->discount != null)
                                            <span class="grand-total"> {{ ($editinvoice->sub_total - $editinvoice->discount) -  $editinvoice->paid_amount }} {{ currency() }}</span>
                                          @elseif ($editinvoice->paid_amount != null)
                                            <span class="grand-total"> {{ $editinvoice->sub_total - $editinvoice->paid_amount }} {{ currency() }}</span>
                                          @elseif($editinvoice->discount != null)
                                            <span class="grand-total"> {{ $editinvoice->sub_total - $editinvoice->discount }} {{ currency() }}</span>
                                          @else
                                            <span class="grand-total"> {{ $editinvoice->sub_total }} {{ currency() }}</span>
                                          @endif
                                        </li>
                                        <li class="list-group-item d-flex justify-content-between">
...
@push('scripts')
    <script>
        ...
        $(document).ready(function() {
            const projectRedirectUrl = "{{ route('admin.purchase.index') }}";
    ...
    <script>
        ...
        <tr class="item">
            ...
            <td>
                <input name="buy_price[]" type="number" class="buy_price" required/>
            </td>
