        ...
        <div class="card px-3 py-3">
            <div class="bg-white border-bottom-0 pb-4 d-flex justify-content-between align-items-center flex-row">
                ...
                <div class="email-template-search-bar btn-group d-flex align-items-center">
                    ...
                    <input type="text" id="datatable-search" class="h-100 border w-100 py-2 px-3" placeholder="{{ __f("Stock Product List Placeholder") }}">
                </div>
            </div>
            <table id="Product_stock" class="display" style="width:100%">
                <thead>
                    <tr>
                        <th>{{ __f("SL Title") }}</th>
                        <th>{{ __f("Product Name Title") }}</th>
                        <th>{{ __f("Product Location Title") }}</th>
                        <th>{{ __f("Invoice Id Title") }}</th>
                        <th>{{ __f("Total Quantity Title") }}</th>
                        <th>{{ __f("Sales Quantity Title") }}</th>
                        <th>{{ __f("Stock Quantity Title") }}</th>
                        <th>{{ __f("Original Buy Price Title") }}</th>
                        <th>{{ __f("Buy Price Title") }}</th>
                        <th>{{ __f("Batch No Title") }}</th>
                    </tr>
                </thead>
                <tbody>
                </tbody>
...
@push('scripts')
    <script>
        ...
        var tables = $('#Product_stock').DataTable({
            processing: true,
            serverSide: true,
            order: [],
            bInfo: true,
            ...
            pageLength: 25,
            ajax: {
                url: "{{ route('admin.stock.product.list.data') }}",
                type: "GET",
                dataType: "JSON",
                data: function(d) {
                    d._token = _token,
            ...
            columns: [{
                ...
                {
                    data: 'product_sales_qty'
                },
                {
                    data: 'stock_qty'
                },
                {
                    data: 'admins_get_total'
                },
                {
                    data: 'buys_price'
                },
