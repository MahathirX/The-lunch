    ...
    <section>
        <div class="card">
            <div class="card-heading">
                <h3 class="p-2">{{ __f("Brand Edit From Title") }}</h3>
            </div>
            <div class="card-body">
                <form id="brandEditForm" action="{{ route('admin.brand.updates', ['id' => $brand->id]) }}"
                    method="POST">
                    @csrf
                    <div class="row">
                        ...
                        <x-form.textbox labelName="{{ __f('Brand Name Label Title') }}" parantClass="col-12 col-md-6" name="name"
                            required="required" placeholder="{{ __f('Brand Name Placeholder Title') }}" errorName="name" class="py-2"
                            value="{!! $brand->name ?? old('name') !!}"></x-form.textbox>
                            ...
                            <option value="1" {{ $brand->status == '1' ? 'selected' : '' }}>{{ __f("Status Publish Title") }}</option>
                            <option value="0" {{ $brand->status == '0' ? 'selected' : '' }}>{{ __f("Status Pending Title") }}</option>
                        </x-form.selectbox>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
...
@push('scripts')
    <script>
        ...
        if (brandImage != '') {
            var myFData = "{{ asset($brand->image ?? '') }}";
            $(function() {
                ImagePriviewUpdate('first__image', 'picture__first', '{{ __f("Brand Image Placeholder Title") }}', myFData);
            });
        }
