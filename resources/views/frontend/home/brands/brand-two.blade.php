@if ($brand && count($brand) > 0)
    <div
        class="container brand-template-two  {{ config('settings.brandnavbarpositionchosevalue') == 1 ? '' : 'brandnavbarpositiontop' }}">
        <h2 class="title title-border position-relative mb-5">{{ config('settings.bands_title') ?? 'Shop by Brands' }}
        </h2>
        <div class="owl-carousel mb-5 owl-simple" data-toggle="owl" data-owl-options='@json(getBrandCarouselOptions())'>
            @foreach ($brand as $brands)
                <div class="brand-template-two-single">
                    ...
                    @endif
                    <h3 class="cat-block-title text-center"> Octavius Garcia </h3>
                </div>
            @endforeach
        </div>
    </div>
